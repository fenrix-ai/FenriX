...
describe('market flow (signing, cutting, hardship, market exclusivity)', () => {
  ...
  it('signPlayer signs an available FA and removes it from the shared market', async () => {
    const market = (await db.doc(`games/${gameId}/market/1`).get()).data();
    const pid = market.available[0];
    const { contract } = await call(signPlayer, { gameId, pid, years: 2 }, 'gmA');
    expect(contract.pid).toBe(pid);
    expect(contract.startRound).toBe(1);
    const team = (await db.doc(`games/${gameId}/teams/${teamA}`).get()).data();
    expect(team.roster.some((c) => c.pid === pid)).toBe(true);
    const marketAfter = (await db.doc(`games/${gameId}/market/1`).get()).data();
    expect(marketAfter.available).not.toContain(pid);
  });

  it('a pid taken off the market by one team cannot be signed by another (no double-booking)', async () => {
    const market = (await db.doc(`games/${gameId}/market/1`).get()).data();
    const pid = market.available[0];
    await call(signPlayer, { gameId, pid, years: 1 }, 'gmA');
    await expect(call(signPlayer, { gameId, pid, years: 1 }, 'gmB')).rejects.toThrow('NOT_IN_MARKET');
    const teamBDoc = (await db.doc(`games/${gameId}/teams/${teamB}`).get()).data();
    expect(teamBDoc.roster.some((c) => c.pid === pid)).toBe(false);
  });

  it('rejects a GM re-submitting the same market signing twice in one phase', async () => {
    const market = (await db.doc(`games/${gameId}/market/1`).get()).data();
    const pid = market.available[0];
    await call(signPlayer, { gameId, pid, years: 1 }, 'gmA');
    // Because a successful sign also removes the pid from market/{round}.available
    // (the fix under test above), a same-team resubmit now trips NOT_IN_MARKET —
    // checked ahead of ALREADY_SIGNED in validateSigning — which is an even earlier
    // and stronger rejection of the same double-sign. ALREADY_SIGNED itself is
    // exercised directly at the pure validateSigning level in market.test.js.
    await expect(call(signPlayer, { gameId, pid, years: 1 }, 'gmA')).rejects.toThrow('NOT_IN_MARKET');
  });

  it('cutRosterPlayer moves a contract to dead money and frees the roster slot', async () => {
    const before = (await db.doc(`games/${gameId}/teams/${teamA}`).get()).data();
    // cut a short (1-round) deal here, keeping the 2-round signing from the first
    // test alive so a later test can confirm it stays off the round-2 market.
    const pid = before.roster.find((c) => c.years === 1).pid;
  ...
  it('hardship fills every under-8 team on FREE_AGENCY exit, never double-booking a pid, and is internally idempotent', async () => {
    ...
    const aPids = a1.roster.map((c) => c.pid);
    const bPids = b1.roster.map((c) => c.pid);
    expect(aPids.filter((pid) => bPids.includes(pid))).toEqual([]); // no pid double-booked
  ...
  it('a signed pid does not reappear in the next round market once its contract is active again', async () => {
    const teamABefore = (await db.doc(`games/${gameId}/teams/${teamA}`).get()).data();
    const stillOwnedPid = teamABefore.roster.find((c) => c.startRound + c.years - 1 >= 2)?.pid;
    expect(stillOwnedPid).toBeDefined(); // the round-1, 2-round signing from the first test above
    ...
    expect(market2.available).not.toContain(stillOwnedPid);
    expect(market2).toHaveProperty('absentCounts');

...
export function SeatPanel({ open, initialTeamId, onClose }: {
  ...
  const seatHolder = (teamId: string, role: Role) =>
    [...players.values()].find((player) => player.teamId === teamId && player.role === role) ?? null;
  const release = async (teamId: string, role: Role) => {
  ...
  return (
    <div className={styles.overlay}>
      <section id="professor-seat-panel" className={styles.drawer} data-testid="seat-panel"
        ...
        {selected && (
          <div className={styles.seatTeam} data-testid={`seats-${selected[0]}`}>
            ...
            <div className={styles.seatRows}>
              {ROLES.map((role) => {
                const holder = seatHolder(selected[0], role);
                const key = `${selected[0]}:${role}`;
                return (

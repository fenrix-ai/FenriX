# W01: Join, franchise identity editor and team lobby Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Join, franchise identity editor and team lobby for the approved Salary Showdown experience.
...
**Branch:** `codex/ss-ui-w01`. **Dev port:** 5183.
...
2. Work on your own `codex/ss-ui-W01` branch in a separate Git worktree. Never implement two packets in one checkout concurrently. Do not use the user's dirty main checkout as a scratch workspace. The integration coordinator supplies the exact base commit after dependencies merge.
...
6. Run tests in your own worktree. Use your allocated Vite port with `npx vite --host 127.0.0.1 --port 5183 --strictPort`. Do not edit Vite configuration to claim another task's port.
...
10. Write `docs/superpowers/plans/salary-showdown-ui-tasks/completions/W01.md` with branch/base/head commits, changed files, test commands/results, screenshot paths, remaining limitations and interface changes (explicitly None if none). Do not edit the shared plan/status table from a worker session.
...
- `app/src/pages/LobbyPage.tsx`
- `app/src/pages/LandingPage.module.css`
- `app/src/pages/LobbyPage.module.css`
- `app/src/components/lobby/IdentityEditor.tsx`
- `app/src/components/lobby/RoleSeats.tsx`
- `app/src/components/lobby/IdentityEditor.test.tsx`
- `app/src/itest/landing.itest.tsx`
- `app/src/itest/lobby.itest.tsx`
- `docs/superpowers/plans/salary-showdown-ui-tasks/completions/W01.md`

## Interfaces

**Consumes:** F01 FranchiseMark/resolveIdentity; F03 useGame().teamSeats; F02 setTeamIdentity/getLobby optional identity.

**Produces:** Three-step joining and a franchise lobby. IdentityEditor({identity:TeamIdentity,onSave:(identity:TeamIdentity)=>Promise<void>,disabled:boolean}) returns ReactElement and remains local to this workstream.
...
- [ ] Extend the targeted tests for this packet's behavior before implementation. Keep existing join/create regressions. Add rejected save preserves preview, team snapshot updates, unclaimed role fallback and phase-change race. Old games without identity must render. Run the targeted command below and confirm the new regression fails for the expected reason.

- [ ] Restructure existing join UI into League → Franchise → Role with a clear progress indicator and back navigation. Retain getLobby polling and late-join server behavior. Preserve typed values on rejection; await create/join before setting active gameId.

- [ ] Create a hero for the own franchise and a three-seat list. Give claimed, own and vacant seats distinct text labels. Animate teammate entry only after the first snapshot; show other franchises in a compact grid without pretending to know their private activity.

- [ ] Place identity editing after successful membership in the lobby. Radio groups select six named accents and three named jerseys; show local preview, Save, pending/error and saved confirmation. Any team member may save during lobby. Do not change createTeam payload or auto-save each click.

- [ ] Preserve renameTeam, role conflicts and name constraints. Explain a taken seat before attempting to join it, but retain server conflict handling if another person takes a seat between polling and click. Phase change disables cosmetic editing and handles a rejected racing save.
...
```bash
npm test -- src/components/lobby/IdentityEditor.test.tsx
npx vitest run --config vitest.integration.config.ts src/itest/landing.itest.tsx src/itest/lobby.itest.tsx
```
...
- [ ] Browser/manual acceptance: Use three browser identities on one franchise and a fourth on another. Inspect 21 teams, all seats claimed, join-in-progress, server error, keyboard steps and reduced motion. Save screenshot evidence with absolute paths in your completion note.

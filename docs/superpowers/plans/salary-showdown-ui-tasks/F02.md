# F02: Additive lobby franchise identity backend Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Additive lobby franchise identity backend for the approved Salary Showdown experience.
...
**Branch:** `codex/ss-ui-f02`. **Dev port:** not applicable.
...
2. Work on your own `codex/ss-ui-F02` branch in a separate Git worktree. Never implement two packets in one checkout concurrently. Do not use the user's dirty main checkout as a scratch workspace. The integration coordinator supplies the exact base commit after dependencies merge.
...
6. Run tests in your own worktree. This backend task needs no Vite server. Do not claim another task's frontend port.
...
10. Write `docs/superpowers/plans/salary-showdown-ui-tasks/completions/F02.md` with branch/base/head commits, changed files, test commands/results, screenshot paths, remaining limitations and interface changes (explicitly None if none). Do not edit the shared plan/status table from a worker session.
...
- `backend/functions/src/teamIdentity.js`
- `backend/functions/src/game.js`
- `backend/functions/index.js`
- `backend/functions/test/team-identity.test.js`
- `backend/SCHEMA.md`
- `docs/superpowers/plans/salary-showdown-ui-tasks/completions/F02.md`

## Interfaces

**Consumes:** The existing auth, game/member/team document paths and callable transaction patterns. The identity enums in this packet are the backend wire contract; no frontend import is necessary.

**Produces:** setTeamIdentity({gameId, identity}) -> {ok:true}; optional identity on getLobby team entries. No existing callable input changes.
...
- [ ] Extend the targeted tests for this packet's behavior before implementation. Use the fixture style in test/rename.test.js. Assert own-team write, rival unchanged, stranger rejected, unauthenticated rejected, malformed gameId rejected, invalid identity rejected, post-start rejected, getLobby old/new shape, and race safety. Assert response {ok:true}. Run the targeted command below and confirm the new regression fails for the expected reason.

- [ ] Implement pure identity validation in teamIdentity.js. Accept only a plain object with exactly accent and jersey enum values; reject arrays, null, arbitrary color strings and extra properties with BAD_IDENTITY. Validate gameId is a nonempty document ID without slash before constructing refs.

- [ ] Add authenticated setTeamIdentity in game.js using the existing region/options. In one transaction read game, caller membership and caller team before any write; require game.status lobby and phase LOBBY, derive teamId exclusively from membership, and update identity only. Treat caller-supplied teamId as unsupported rather than using it.

- [ ] Export the callable from index.js. Extend getLobby responses only with validated stored identity when present. Preserve existing public field list; do not include private bids, membership uids or roster data.

- [ ] Document optional metadata and the new callable in backend/SCHEMA.md. Preserve old games and newTeamDoc defaults; do not migrate existing records or widen Firestore write rules.

- [ ] Exercise concurrent identity-save and startSeason to ensure no identity write can occur after the transition wins. Use existing emulator transaction tests as the harness; do not reset shared emulator data.
...
export function isTeamIdentity(value) {
  return value !== null && typeof value === 'object'
    ...
    && ACCENTS.has(value.accent) && JERSEYS.has(value.jersey);
}
// Add to team-identity.test.js with its existing Vitest imports.
// expect(isTeamIdentity({ accent: 'teal', jersey: 'stripe' })).toBe(true);
// expect(isTeamIdentity({ accent: '#fff', jersey: 'stripe' })).toBe(false);
// expect(isTeamIdentity({ accent: 'teal', jersey: 'stripe', url: 'x' })).toBe(false);
```

### Verification commands

```bash
npm test -- test/team-identity.test.js test/rename.test.js test/create-team.test.js test/advance-race.test.js
```
...
- [ ] Browser/manual acceptance: Backend working directory is backend/functions. Run against the reserved DEV emulator only. Start a fresh two-team lobby and confirm a new client receives saved identity. No frontend visual changes belong here. Save screenshot evidence with absolute paths in your completion note.

# W04: Auction showcase and sealed offer workflow Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Auction showcase and sealed offer workflow for the approved Salary Showdown experience.
...
**Branch:** `codex/ss-ui-w04`. **Dev port:** 5186.
...
2. Work on your own `codex/ss-ui-W04` branch in a separate Git worktree. Never implement two packets in one checkout concurrently. Do not use the user's dirty main checkout as a scratch workspace. The integration coordinator supplies the exact base commit after dependencies merge.
...
6. Run tests in your own worktree. Use your allocated Vite port with `npx vite --host 127.0.0.1 --port 5186 --strictPort`. Do not edit Vite configuration to claim another task's port.
...
10. Write `docs/superpowers/plans/salary-showdown-ui-tasks/completions/W04.md` with branch/base/head commits, changed files, test commands/results, screenshot paths, remaining limitations and interface changes (explicitly None if none). Do not edit the shared plan/status table from a worker session.
...
- `app/src/pages/AuctionPage.tsx`
- `app/src/pages/AuctionPage.module.css`
- `app/src/components/auction/OfferCard.tsx`
- `app/src/lib/auctionDraft.ts`
- `app/src/lib/auctionDraft.test.ts`
- `app/src/itest/auction.itest.tsx`
- `docs/superpowers/plans/salary-showdown-ui-tasks/completions/W04.md`

## Interfaces

**Consumes:** F01 PlayerCard/action receipts; existing private own-team auction snapshot and submitBids callable.

**Produces:** Offer cards and offer summary; mergeAuctionSnapshot(draft:Bids,saved:Bids,dirty:ReadonlySet<string>):Bids, where Bids = Record<string,{rate:number;years:number}> in lib/auctionDraft.ts.
...
- [ ] Extend the targeted tests for this packet's behavior before implementation. Add two-client snapshot update while typing, edit during pending save, withdrawal {}, server rejection without SEALED, legal over-cap submission, and phase change while awaiting success. Run the targeted command below and confirm the new regression fails for the expected reason.

- [ ] Lay out showcase cards with raw player facts, rate and duration. Add an always-visible summary of this team's proposed guaranteed commitments and round exposure. Animate numeric changes decoratively without announcing every interpolated value.

- [ ] Keep a dirty set per pid and merge remote saved values only into clean entries. Track submission generation: clear dirty flags only for values matching the exact payload acknowledged. Remote updates or an older response must not erase newer typing.

- [ ] Validate existing min rate, increment and years rules. Keep over-cap exposure as warning, not prohibition. Submit {} to withdraw all bids, never null. Disable only actual invalid or in-flight submission states.

- [ ] Show brief SEALED treatment after successful submit and persist clear Saved/Unsaved status. Revisions remain available. Public resolution belongs to Results; do not query rivals' private documents or infer leading bids.
...
export function mergeAuctionSnapshot(draft: Bids, saved: Bids, dirty: ReadonlySet<string>): Bids {
  const result = { ...saved };
  for (const key of dirty) {
    if (Object.prototype.hasOwnProperty.call(draft, key)) result[key] = draft[key];
    else delete result[key];
  }
  return result;
}
// lib/auctionDraft.test.ts
import { expect, test } from 'vitest';
import { mergeAuctionSnapshot } from './auctionDraft';
test('snapshot preserves dirty edits and dirty removal', () => {
  expect(mergeAuctionSnapshot({ a: { rate: 8, years: 2 } },
    { a: { rate: 3, years: 1 }, b: { rate: 2, years: 1 } }, new Set(['a', 'b'])))
...
```bash
npm test -- src/lib/auctionDraft.test.ts
npx vitest run --config vitest.integration.config.ts src/itest/auction.itest.tsx
```
...
- [ ] Browser/manual acceptance: Inspect 1/3/many stars, large guarantees, all bids withdrawn, reduced motion and laptop keyboard controls. Verify no private information appears on public surfaces. Save screenshot evidence with absolute paths in your completion note.

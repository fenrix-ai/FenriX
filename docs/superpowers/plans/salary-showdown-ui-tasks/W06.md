# W06: Simulation broadcast and completed matchup inspection Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Simulation broadcast and completed matchup inspection for the approved Salary Showdown experience.
...
**Branch:** `codex/ss-ui-w06`. **Dev port:** 5188.
...
2. Work on your own `codex/ss-ui-W06` branch in a separate Git worktree. Never implement two packets in one checkout concurrently. Do not use the user's dirty main checkout as a scratch workspace. The integration coordinator supplies the exact base commit after dependencies merge.
...
6. Run tests in your own worktree. Use your allocated Vite port with `npx vite --host 127.0.0.1 --port 5188 --strictPort`. Do not edit Vite configuration to claim another task's port.
...
10. Write `docs/superpowers/plans/salary-showdown-ui-tasks/completions/W06.md` with branch/base/head commits, changed files, test commands/results, screenshot paths, remaining limitations and interface changes (explicitly None if none). Do not edit the shared plan/status table from a worker session.
...
- `app/src/pages/SimulatePage.tsx`
- `app/src/pages/SimulatePage.module.css`
- `app/src/components/simulation/MatchReveal.tsx`
- `app/src/components/simulation/CompletedGames.tsx`
- `app/src/components/simulation/MatchBoxscore.tsx`
- `app/src/lib/matchRows.ts`
- `app/src/lib/matchRows.test.ts`
- `app/src/itest/simulate.itest.tsx`
- `docs/superpowers/plans/salary-showdown-ui-tasks/completions/W06.md`

## Interfaces

**Consumes:** F03 useRoundPresentation and its single progression clock; existing parseBoxCsv, GameResult, BoxRow and team identities.

**Produces:** Simulation display with no new clock or server mutation. matchRows(csv:string,gameId:string):BoxRow[] selects a whole matchup.
...
- [ ] Extend the targeted tests for this packet's behavior before implementation. Use a real 23-column fixture from existing boxfeed tests for matchRows.test.ts: two game_ids and duplicate team names must still return just the selected full match. Extend simulate.itest.tsx to assert final score absent before prefix and present afterward, record consistency and missing-document loading. Run the targeted command below and confirm the new regression fails for the expected reason.

- [ ] Show a meaningful preparing-results state when rd is null; keep team identity and current round visible. Once ready, present the next matchup intro and then actual homeScore/awayScore when the shared applied prefix advances. Never interpolate fictional game events or show scores in hidden aria text.

- [ ] Derive own-team record and completed games from the shared applied prefix. Render a completed-game strip with clear opponent/date-free round matchup labels and selected expansion. Reveal all calls provider.revealAll and does not advance the game phase.

- [ ] Expand only completed matchups. Parse actual CSV and select by game_id; show real boxscore fields. Resolve team identity through GameResult IDs; if names collide, display the full matchup and a neutral notice rather than assigning ambiguous rows to the player's team.

- [ ] Cancel local intro decoration on round change. Do not create separate progression timers for the strip, record or score. A shared provider state update drives all three. Preserve errors and skip/reduced-motion behavior.
...
// JSX integration inside SimulatePage, after reading the provider:
// const visibleGames = presentation.rd?.games.slice(0, presentation.applied) ?? [];
// <button onClick={presentation.revealAll} disabled={presentation.complete}>
//   Reveal all results
// </button>
```
...
```bash
npm test -- src/lib/matchRows.test.ts src/lib/liveStandings.test.ts
npx vitest run --config vitest.integration.config.ts src/itest/simulate.itest.tsx
```
...
- [ ] Browser/manual acceptance: Inspect two teams and 21 teams, tab background/return, route away/back, Reveal all, reduced motion, own-team idle gap and duplicate franchise names. No invented play-by-play. Save screenshot evidence with absolute paths in your completion note.

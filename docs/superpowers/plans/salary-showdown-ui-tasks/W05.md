# W05: Court lineup interaction and accessible placement Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Court lineup interaction and accessible placement for the approved Salary Showdown experience.
...
**Branch:** `codex/ss-ui-w05`. **Dev port:** 5187.
...
2. Work on your own `codex/ss-ui-W05` branch in a separate Git worktree. Never implement two packets in one checkout concurrently. Do not use the user's dirty main checkout as a scratch workspace. The integration coordinator supplies the exact base commit after dependencies merge.
...
6. Run tests in your own worktree. Use your allocated Vite port with `npx vite --host 127.0.0.1 --port 5187 --strictPort`. Do not edit Vite configuration to claim another task's port.
...
10. Write `docs/superpowers/plans/salary-showdown-ui-tasks/completions/W05.md` with branch/base/head commits, changed files, test commands/results, screenshot paths, remaining limitations and interface changes (explicitly None if none). Do not edit the shared plan/status table from a worker session.
...
- `app/src/pages/LineupPage.tsx`
- `app/src/pages/LineupPage.module.css`
- `app/src/components/lineup/CourtSlot.tsx`
- `app/src/components/lineup/LineupRoster.tsx`
- `app/src/components/lineup/PlacementStatus.tsx`
- `app/src/components/lineup/CourtSlot.test.tsx`
- `app/src/itest/lineup.itest.tsx`
- `docs/superpowers/plans/salary-showdown-ui-tasks/completions/W05.md`

## Interfaces

**Consumes:** Existing slots.place/fromLineup/toLineup and dnd-kit; F01 player/franchise primitives and receipts; exact PLAYSTYLES and PLAYSTYLE_BLURBS.

**Produces:** Large court, click/keyboard placement and clear depth zones. CourtSlot({label:string,pid:number|null,eligible:boolean,onPlace:()=>void,children?:ReactNode}) returns ReactElement.
...
- [ ] Extend the targeted tests for this packet's behavior before implementation. Keep slots.test.ts unchanged unless an actual pre-existing bug blocks approved interaction; report such blockers before expanding ownership. Add UI tests for legal swap, rejected swap, depth preservation, click path and locked-remount regression. Run the targeted command below and confirm the new regression fails for the expected reason.

- [ ] Grow court to use laptop height alongside roster and tactics; size responsively rather than a fixed 240px court. Distinguish five starters, sixth, two active bench spots and inactive depth. Keep all depth pids and original order.

- [ ] Use the existing place function as the single legality check for drag and click paths. Selecting a player highlights legal target slots; occupied-slot legality checks both destination and displaced player's original slot. Invalid action leaves state unchanged and explains the position conflict in a status region.

- [ ] Expose every slot as a keyboard-operable button. Select player then choose slot, Escape cancels selection; after placement retain a logical focus location and announce outcome. Drag overlay and lift are supplemental, not necessary to operate.

- [ ] Animate accepted local swaps without submitting automatically. Submit/lock uses the existing callable and scoped server acknowledgment. Initialize locked lineups verbatim on remount; never auto-arrange them. Do not discard unplayed bench depth.

- [ ] Keep playstyle text verbatim and readable on dark surfaces. Selection may alter court line emphasis or decoration, but must not show tactical projections, boosted stats or invented synergy.

### Critical code or regression contract

```tsx
// components/lineup/CourtSlot.test.tsx
...
```bash
npm test -- src/components/lineup/CourtSlot.test.tsx src/lib/slots.test.ts
npx vitest run --config vitest.integration.config.ts src/itest/lineup.itest.tsx
```
...
- [ ] Browser/manual acceptance: Finish and revise a lineup using only keyboard, then using trackpad drag. Check 8-player and deeper rosters, role fallback, ready status still editable, long names and 200% zoom. Save screenshot evidence with absolute paths in your completion note.

...
- **Hard salary cap per round: $100M** (tunable). Payroll = sum of active contract rates + dead money. The game blocks any signing/bid that would exceed the cap in any remaining round. No debt mechanic. **No exceptions as of the 2026-07-26 playtest-polish spec (§2):** hardship auto-signs are no longer cap-exempt because they are no longer priced — every hardship slot is filled by a synthetic $0 "Default Role Player", so payroll can never be pushed over the cap by the autofill.
...
- **Roster limits:** min 8 (must cover 2G/2W/1B lineup), max 10 — the max is enforced at FA signing, at bid input where knowable, and at auction resolution. Round-1 timeout with an illegal roster → the hardship autofill (§13) signs synthetic **"Default Role Player"** entries on 1-round, **$0** deals — replacement-level and identical for every team, never real free agents (2026-07-26 playtest-polish spec §2; they are reserved pids outside the 175-player pool, so there is nothing to farm and no cap exemption to grant). The same hardship rule guarantees a legal roster in rounds 2–5.
...
**Hardship rule (legal-roster guarantee, all rounds):** evaluated at Free Agency close each round — if a team cannot field a legal 8-man / 2G-2W-1B roster (expirations, cuts, dead money, timeout — any cause), the system auto-signs synthetic **"Default Role Player"** entries on **1-round, $0 deals**, flagged on the team's screen. The round-1 illegal-roster autofill is this same mechanism.

> **Superseded 2026-07-26 (playtest-polish spec §2, Dylan-adjudicated).** This rule previously signed *the cheapest legal real free agents, cap-exempt if necessary*. It no longer touches the free-agent pool at all: the fill is a fixed set of reserved-pid (9001+) synthetic players, replacement-level and **identical for every team**, deficit positions first and still bounded by the 10-man max. Rationale: the old rule let a stranded team farm cheap real talent and could push a displayed payroll past the cap; a $0 synthetic can do neither. Consequences: hardship contracts are excluded from the Bargain award and from finale best/worst-signing (a $0 contract teaches no signing lesson) but still appear in box scores and are eligible for the box-derived MVP / top-scorer awards; total spend is unaffected because their committed money is zero. Their engine ratings are the empirical 25th percentile of the real 175 across every strength and box input, supplied as an additive overlay that leaves the generated dataset and the Python engine-parity fixture untouched. They are also never signable through any callable — validateSigning rejects the whole reserved pid range and expiringPids filters them out — so hardship is the ONLY way one ever reaches a roster.

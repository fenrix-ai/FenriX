...
2. **Free Agency** (~2.5 min) — GM submits. **Rotating market:** round 1 opens with a large pool (~75% of all free agents); each later round the signable list is a fresh server-seeded random subset (~40–50%) — identical for every team. Unavailable players are hidden by default (no clutter); an "All players" filter chip reveals them greyed for reference. Displayed prices are always **tonight's ask** (base list × inflation for the current round) — the CSV carries the base, and noticing the drift is the students' arithmetic, not a UI callout. Anti-frustration guard: a player absent two consecutive rounds is guaranteed into the next draw. Cut/walked players join the rotation; expiring re-signs are exempt (incumbent right lives in Front Office). Rotation × inflation is the urgency engine: waiting risks both reappearance and +8% pricing. Live season stats shown beside sticker stats. Sign flow = years slider (see §5). Signings commit instantly (shared pool — no inter-team conflict). Round 1 variant: extended timer, build min-8 roster, roster checklist widget enforces position coverage.
3. **Star Auction** (~2 min) — Scout submits. 4–6 exclusive star cards (stats, age, hype, grade, followers — no price). Sealed bid = a **contract offer**: per-round salary + years slider (1 to rounds remaining), same mental model as the FA sign drawer. **Winner = highest total guaranteed money** (salary × years); you pay your own offered structure; ties broken randomly. Live cap validation per covered round; an exposure meter shows worst-case payroll if all your bids win. Teams may bid on multiple stars and may win multiple if cap fits. Public reveal shows the winning structure ("$8.0M/rd × 3 = $24.0M") — price and term discovery. Round 5 degenerates gracefully to a pure per-round bid.
4. **Set Lineup** (~1.5 min) — Coach submits. 5 starter slots (2 Guards / 2 Wings / 1 Big, enforced), 1 Sixth Man, rest Bench — plus a **playstyle pick** (one of five, Balanced pre-selected; see §8a). Student-facing playstyle descriptions are one plain sentence of behavior, never advice: Balanced "Play your normal game." · Run & Gun "Play fast. More shots." · Three-Point Barrage "Shoot more threes." · Inside Attack "Feed your Big." · Lockdown "Slow it down. Defend." No derived synergy meters shown (would leak the hidden model). Timeout → last valid lineup + playstyle carries over (round 1: auto-arrangement, Balanced).
...
- **Auction resolution:** at phase close, sort all bids by guaranteed total descending; award each star to its highest bidder whose cap still fits across every covered round (bids that no longer fit after an earlier win are skipped); ties broken randomly. Losers pay nothing. Winning structures are public (price and term discovery).
...
**`league_history.csv`** — 90 rows (30 fictional teams × 3 seasons; `season` column). Columns: `team_name`, `season`, `wins`, `losses`, team-level `pts_per_game`, `fg_pct`, `3p_pct`, `ft_pct`, `rebounds`, `assists`, `steals`, `blocks`, `turnovers`, `total_payroll`, `avg_hype`, `playstyle` (appended; the team's predominant playstyle that season — fuels the interaction-term discovery in §9.7).
...
2a. **Playstyle modifiers (applied to team strength before games):** the Coach's per-round playstyle re-weights contributions — every playstyle is an amplifier whose value depends entirely on roster fit; none is good on average (§10.8):
   - **Balanced** — baseline, no modifiers
   - **Run & Gun** — possessions +15%; scoring contributions ×1.15; **turnover damage ×1.5**; slightly lower game variance (more possessions)
   - **Three-Point Barrage** — starters' offense re-weighted toward 3-point skill (×2), interior scoring ×0.7; misfires without 3+ genuine shooters; slightly higher variance
   - **Inside Attack** — Big's offensive contribution ×1.4, rebounding weight up, guard scoring ×0.85
   - **Lockdown** — possessions −15%; defensive contributions ×1.4; scoring ×0.9; **higher game variance** (fewer possessions → more upsets — the rational underdog play)
...
`scout_grade` is generated from TrueImpact + heavy noise (realized r ≈ 0.35) — the "experts beat nothing, models beat experts" column. `personality` is assigned independently of everything (pure flavor; the reveal shows it predicted nothing).

Market sanity is a **two-regime design** (amended during dataset generation): among *ordinary* players (excluding traps and defensive specialists) salary tracks TrueImpact at **R² ≈ 0.5** — the market isn't stupid. Across the *full* pool the correlation collapses toward zero, because the exceptions ARE the game: traps overpriced, defenders underpriced. ≥15 players sit in bottom-half salary AND top-quartile TrueImpact (the treasure cluster, harness-enforced).
...
| Age | Drift |
|---|---|
| 19–23 | +4%/yr, capped +12% (Young Risers outperform sticker) |
| 24–29 | flat (stats honest) |
| 30–32 | −4%/yr |
| 33+ | the cliff: ×0.76 at 33, ×0.66 at 34, ×0.56 at 35, floor ×0.42 (Aging Legends are 33–37) |
...
| Playstyle | Moderator (published column) | Designed fingerprint in history |
|---|---|---|
| Run & Gun | `turnovers` | low-TO Run & Gun teams won 55+; high-TO won ~25 |
| 3PT Barrage | `3p_pct` | soared with elite 3P%, cratered without |
| Inside Attack | `rebounds` (team-level Big-strength proxy; finer read via own roster in players.csv) | paint-pounding needs a real Big |
| Lockdown | `steals` + `blocks` | slow pays only for defense-first rosters |
...
4. **Free Agency (`/game/market`)** — §4.2. New UI build. Approved layout (mocked 2026-07-14): **analyst table** — sortable columns (Player, Pos, Age, Hype, $/rd, PPG, FGA, FG%, 3P%, REB, AST, TOV), filter chips (In market tonight [default] · All players [greyed reference] · position · price · search), sticky payroll bar. Row click opens the **sign drawer** (right rail): years slider with per-round rate per stop, total commitment line, live cap check, confirm button. Card-grid layout rejected for this screen (cards remain the Front Office language).
...
- Trap-star count (6–8), treasure-cluster size (≥15), ordinary-player salary R² (~0.5), league-table regression R²
- Scout-grade correlation (r ≈ 0.3); follower-count weight in hype (0.20)
- All generator dials live in `games/salary-showdown/datagen/config.py` — the single source of truth; the spec records design intent, config records tuned reality
- Market rotation: round-1 pool share (~75%), per-round draw share (~40–50%), reappearance-guarantee window (2 rounds)
- Playstyle multipliers (pace ±15%, TO damage ×1.5, 3pt re-weight ×2, Big boost ×1.4, defense ×1.4, variance shifts)
- Phase timer durations; auction wave sizes (4–6)

## 16. Dataset v1.0 — Generation Results (2026-07-14)

Generated by `games/salary-showdown/datagen/` (seed 310); all 9 harness checks green.
...
- Base student regression on the league table: turnovers **−5.3 wins each** (p<0.001), FG% strongly positive (p=0.011), payroll t=−0.4, hype t=1.3 (both null, as designed), R² = 0.68
- Fairness: modeler top-3 **82%**, champion **52%** (k = 0.09); scout-grade strategy median rank 4 of 20 — between modelers and PPG-sorters
- Bargain cluster: 15 players; trap gap: min hype-vs-TrueImpact percentile gap **75 points** (13 traps)
- All four playstyle interactions recoverable at p<0.05 (RG×TOV −2.6, BAR×3P +376, INS×REB +2.3, LOCK×DEF +3.1) with flat main effects (max |t| = 0.33)
- Best-fit style across 90 history rosters: Balanced 11 / Run & Gun 13 / Barrage 26 / Inside 19 / Lockdown 21 — every style is somebody's answer

Canon established by the data: **the $100M cap is new this season** — historical payrolls
were uncapped (up to ~$120M), which is both the cover story for history payroll levels and
...
sanity (§9.1), steeper aging cliff with Legends at 33–37 (§9.3), volume-creation term in
scoring value (small real value per attempt — volume is overpriced, not worthless),

...
- **Misfits must exist:** ~60% of historic teams run a fitting playstyle, ~40% a misfitting one (bad coaches are realistic and statistically necessary — no contrast, no discoverable interaction). Stratified so each playstyle gets ~18 teams spanning low/mid/high values of its moderator.
...
8. **No dominant playstyle:** in fairness sims and in generated history, no playstyle beats Balanced unconditionally (mean wins within ±2), and every playstyle is the best choice for at least one plausible roster archetype.
9. **Interaction detectability (concrete assertions, run on the actual generated 90 rows):**
   - Fit `wins ~ stats + playstyle dummies + playstyle × moderator terms`: every designed interaction coefficient (§9.7 table) has the designed sign at p < 0.05.
   - Playstyle main effects stay flat: each playstyle dummy's CI includes 0; mean wins per playstyle within ±2 of Balanced.
   - Moderator spread exists within every playstyle cell (low/mid/high terciles all populated).
